// scripts/export-products-full2.js
/**
 * Export all product data from seed packs (including dynamically built products) to Markdown.
 * Uses ts-node with `skipProject:true` to ignore the project's tsconfig (avoids rootDir errors).
 */
...
function main() {
  ...
  for (const file of files) {
    ...
    // Dynamically import the TS module (ts-node will transpile on the fly)
    const packModule = require(modulePath);
    // Find exported variable that ends with 'SeedPack'
    const exportKey = Object.keys(packModule).find(k => k.toLowerCase().endsWith('seedpack'));
    if (!exportKey) continue;
    const pack = packModule[exportKey];
