...
function PackagingHeader({ companyName }: { companyName: string }) {
  const theme = getTemplateTheme("template-10");
  return (
    <header
    ...
    >
      <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between gap-8 px-6">
        <Link href="/" className="flex items-center gap-2.5 text-base font-semibold tracking-tight" style={{ color: theme.accent }}>
          <span className="flex h-7 w-7 items-center justify-center rounded bg-purple-500/15 text-xs font-bold text-purple-300">PK</span>
...
export function Template10Layout({ children, settings, categories }: PublicLayoutProps) {
  ...
  return (
    <div className="relative flex min-h-screen flex-col" style={{ backgroundColor: theme.surface, color: theme.accent }}>
      <TrackingProvider />
      <PackagingHeader companyName={settings.companyNameEn} />
