...
function ComponentHeader({ companyName }: { companyName: string }) {
  const theme = getTemplateTheme("template-08");
  return (
    <header
    ...
    >
      <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between gap-8 px-6">
        <Link href="/" className="flex items-center gap-2.5 text-base font-semibold tracking-tight" style={{ color: theme.accent }}>
          <span className="flex h-7 w-7 items-center justify-center rounded bg-indigo-500/15 text-xs font-bold text-indigo-300">CP</span>
...
export function Template08Layout({ children, settings, categories }: PublicLayoutProps) {
  ...
  return (
    <div className="relative flex min-h-screen flex-col" style={{ backgroundColor: theme.surface, color: theme.accent }}>
      <TrackingProvider />
      <ComponentHeader companyName={settings.companyNameEn} />
