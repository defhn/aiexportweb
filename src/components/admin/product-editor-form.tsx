import { BarChart3, FileText, Globe2, Images, Package, Save, Settings2, Sparkles } from "lucide-react";
...
function SidebarSwitch({ name, label, checked }: { name: string; label: string; checked: boolean }) {
  return (
...
export function ProductEditorForm({
  ...
  return (
    <div className="mx-auto max-w-[1400px] pb-40">
      ...
      <form action={action} id="product-editor-form" className="grid gap-8 lg:grid-cols-[1fr_380px]">
        ...
        <div className="space-y-8">
          <section className="rounded-[2rem] border border-stone-200 bg-white p-8 shadow-sm">
            ...
            <div className="grid gap-6">
              <div className="grid gap-6 md:grid-cols-2">
                <label className="block text-sm font-medium text-stone-700">
                  产品分类
                  <select className={inputClassName} defaultValue={product.categoryId ?? ""} name="categoryId">
                ...
                <label className="block text-sm font-medium text-stone-700">
                  排序值
                  <input className={inputClassName} defaultValue={product.sortOrder} name="sortOrder" type="number" />
                </label>
              </div>
              ...
              <label className="block text-sm font-medium text-stone-700">
                URL Slug
                <input className={inputClassName} defaultValue={product.slug} name="slug" placeholder="custom-aluminum-cnc-bracket" />
              </label>

              <div className="grid gap-6 md:grid-cols-2">
                <label className="block text-sm font-medium text-stone-700">
                  中文短描述
                  <textarea className={textareaClassName} defaultValue={product.shortDescriptionZh} name="shortDescriptionZh" />
                </label>
                <label className="block text-sm font-medium text-stone-700">
                  Short Summary (EN)
                  <textarea className={textareaClassName} defaultValue={product.shortDescriptionEn} name="shortDescriptionEn" />
                </label>
              </div>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <label className="block text-sm font-medium text-stone-700">
                  中文详情
                  <textarea className={`${textareaClassName} min-h-56`} defaultValue={product.detailsZh} name="detailsZh" />
                </label>
                <label className="block text-sm font-medium text-stone-700">
                  Full Details (EN)
                  <textarea className={`${textareaClassName} min-h-56`} defaultValue={product.detailsEn} name="detailsEn" />
                </label>
              </div>
            </div>
            ...
            <div className="mt-10 rounded-[2rem] border border-blue-100 bg-blue-50 p-6">
              ...
              <ProductAiTools categories={categories} formId="product-editor-form" gate={productAiGate} />
            </div>
          </section>
          ...
          <section className="rounded-[2rem] border border-stone-200 bg-white p-8 shadow-sm">
            ...
            <div className="space-y-4">
              {product.defaultFields.map((field) => (
                <div key={field.fieldKey} className="grid gap-4 rounded-3xl border border-stone-100 p-6 md:grid-cols-[1fr_1fr_auto]">
                  <label className="block text-sm font-medium text-stone-700">
                    {field.labelZh}
                    <input className={inputClassName} defaultValue={field.valueZh} name={`field-${field.fieldKey}__valueZh`} />
                  </label>
                  <label className="block text-sm font-medium text-stone-700">
                    {field.labelEn}
                    <input className={inputClassName} defaultValue={field.valueEn} name={`field-${field.fieldKey}__valueEn`} />
                  </label>
                  <label className="flex items-center gap-2 self-end pb-3 text-sm font-medium text-stone-700">
                    <input defaultChecked={field.isVisible} name={`field-${field.fieldKey}__isVisible`} type="checkbox" />
                    前台显示
                  </label>
                </div>
              ))}
            ...
            <div className="mt-10">
              ...
              <div className="space-y-4">
                {customFields.map((field, index) => (
                  <div key={`custom-${index + 1}`} className="grid gap-4 rounded-3xl border border-stone-100 p-6 md:grid-cols-2">
                    <label className="block text-sm font-medium text-stone-700">
                      字段名称（中文）
                      <input className={inputClassName} defaultValue={field.labelZh} name={`custom-${index}__labelZh`} />
                    </label>
                    <label className="block text-sm font-medium text-stone-700">
                      Label (EN)
                      <input className={inputClassName} defaultValue={field.labelEn} name={`custom-${index}__labelEn`} />
                    </label>
                    <label className="block text-sm font-medium text-stone-700">
                      字段值（中文）
                      <input className={inputClassName} defaultValue={field.valueZh} name={`custom-${index}__valueZh`} />
                    </label>
                    <label className="block text-sm font-medium text-stone-700">
                      Value (EN)
                      <input className={inputClassName} defaultValue={field.valueEn} name={`custom-${index}__valueEn`} />
                    </label>
                    <label className="flex items-center gap-2 text-sm font-medium text-stone-700 md:col-span-2">
                      <input defaultChecked={field.isVisible} name={`custom-${index}__isVisible`} type="checkbox" />
                      前台显示
                    </label>
                  </div>
                ))}
          ...
          <section className="rounded-[2rem] border border-stone-200 bg-white p-8 shadow-sm">
            <div className="mb-8 flex items-center gap-3">
              ...
              <h3 className="text-xl font-bold text-stone-900">图片与资料</h3>
            </div>
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-2">
              <label className="block text-sm font-medium text-stone-700">
                资料 PDF
                <select className={inputClassName} defaultValue={product.pdfFileId ?? ""} name="pdfFileId">
            ...
            <div className="mt-10">
              <p className="mb-4 text-sm font-semibold text-stone-900">产品图库（可多选）</p>
              {imageAssets.length ? (
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {imageAssets.map((asset) => (
                    <label key={asset.id} className="overflow-hidden rounded-2xl border border-stone-200 bg-white">
                      ...
                      <div className="space-y-3 p-4">
                        ...
                        {product.id && galleryIds.has(asset.id) ? (
                          <GalleryReplaceButton currentMediaId={asset.id} productId={product.id} />
                        ) : null}
                      </div>
                    </label>
                  ))}
        ...
        <div className="space-y-8">
          <section className="rounded-[2rem] bg-slate-950 p-8 text-white shadow-sm">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 rounded-3xl border border-white/5 bg-white/5 p-6">
                <SidebarSwitch checked={product.isFeatured} label="设为推荐产品" name="isFeatured" />
                <SidebarSwitch checked={product.showInquiryButton} label="显示询盘按钮" name="showInquiryButton" />
                <SidebarSwitch checked={product.showWhatsappButton} label="显示 WhatsApp 按钮" name="showWhatsappButton" />
                <SidebarSwitch checked={product.showPdfDownload} label="允许下载资料" name="showPdfDownload" />
              </div>

              <button type="submit" className="flex h-14 w-full items-center justify-center gap-2 rounded-full bg-blue-600 text-sm font-bold text-white hover:bg-blue-500">
          ...
          <section className="rounded-[2rem] border border-stone-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <label className="block text-sm font-medium text-stone-700">
                Meta Description
                <textarea className={textareaClassName} defaultValue={product.seoDescription} name="seoDescription" />
              </label>
            </div>
          </section>
        </div>
