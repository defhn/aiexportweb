  ...
  return (
    <div className="rounded-2xl bg-stone-50 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm font-medium text-stone-950">AI 英文文案辅助</p>
        {gate.status === "trial" && remaining !== null ? (
          <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-800">
            试用剩余 {remaining}/{gate.limit}
          </span>
        ) : null}
      </div>
      ...
      <p className="mt-2 text-sm leading-6 text-stone-600">
        {gate.status === "included"
          ? "可直接生成英文产品标题、产品描述和 SEO 文案。"
          : gate.status === "trial" && !isLocked
            ? "当前套餐可试用少量 AI 文案功能，试用用完后可升级到 AI 销售版。"
            : "当前套餐暂不可继续使用 AI 产品文案，可联系你升级更高版本。"}
      </p>
