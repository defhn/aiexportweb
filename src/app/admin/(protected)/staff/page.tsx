...
import { Loader2, Plus, Trash2, KeyRound, Users } from "lucide-react";
...
export default function StaffPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admin/staff", {
        ...
        body: JSON.stringify({ username: newUsername, password: newPassword, role: "employee" }),
      });
      const data = await res.json();
  ...
  const handleDelete = async (id: number, username: string) => {
    if (!confirm('确定要删除员工 ' + username + '？此操作不可恢复。')) return;
    try {
    ...
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-stone-200 bg-white p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-2xl font-semibold text-stone-950">员工账号管理</h2>
        </div>
        <p className="text-sm text-stone-500 leading-6">管理可登录 Admin 后台的员工账号，密码每次创建后无法查询，请妥善保管。</p>
      </section>

      {successMessage && (
        <div className="rounded-2xl bg-green-50 px-5 py-3 text-sm font-medium text-green-700">{successMessage}</div>
      )}
      {error && (
      ...
      <section className="rounded-[1.5rem] border border-stone-200 bg-white p-6 shadow-sm">
        <h3 className="mb-4 text-base font-semibold text-stone-950">添加新员工</h3>
        <form onSubmit={handleCreate} className="flex flex-wrap gap-3">
          <input
            value={newUsername}
            onChange={(e) => setNewUsername(e.target.value)}
            required minLength={3} placeholder="用户名（至少3位）"
          ...
          <input
            ...
            required minLength={6} placeholder="密码（至少6位）"
            className="min-w-48 flex-1 rounded-xl border border-stone-300 px-4 py-2.5 text-sm outline-none focus:border-blue-500"
          />
      ...
      <section className="overflow-hidden rounded-[1.5rem] border border-stone-200 bg-white shadow-sm">
        <div className="border-b border-stone-200 px-6 py-4">
          <h3 className="text-base font-semibold text-stone-950">当前员工 ({employees.length})</h3>
        </div>
        {loading ? (
          ...
          <div className="py-12 text-center text-sm text-stone-400">暂无员工账号</div>
        ) : (
          <div className="divide-y divide-stone-100">
            {employees.map((emp) => (
              <div key={emp.id} className="relative flex items-center justify-between px-6 py-4">
                <div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { setResetId(emp.id); setResetPassword(""); }}
                  ...
                  <button onClick={() => handleDelete(emp.id, emp.username)}
                ...
                {resetId === emp.id && (
                  ...
                  >
                    <p className="mb-3 text-sm font-semibold text-stone-900">为「{emp.username}」设置新密码</p>
                    <input type="password" required minLength={6} value={resetPassword}
                      onChange={(e) => setResetPassword(e.target.value)} placeholder="至少 6 位"
                      className="mb-3 w-full rounded-xl border border-stone-300 px-3 py-2 text-sm outline-none focus:border-blue-500"
                    />
                    <div className="flex gap-2">
                      <button type="submit" className="flex-1 rounded-lg bg-blue-600 py-1.5 text-sm font-semibold text-white hover:bg-blue-700">确认重置</button>
                      <button type="button" onClick={() => setResetId(null)} className="flex-1 rounded-lg border border-stone-200 py-1.5 text-sm text-stone-600 hover:bg-stone-50">取消</button>
                    </div>
                  </form>
                )}
              </div>
